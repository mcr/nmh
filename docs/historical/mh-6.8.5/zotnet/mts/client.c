...
#ifndef	lint
static char ident[] = "@(#)$Id: client.c,v 1.22 1995/12/06 22:15:26 jromine Exp shettich $";
#endif	/* lint */
...
#ifdef __STDC__
static int rcaux (struct servent *, struct hostent *, char *);
...
int	client (args, protocol, service,  response)
char   *args,
       *protocol,
	...
	if (**ap == '\01') {
		...
		while (hp = gethostent ())
		    if (np -> n_addrtype == hp -> h_addrtype
			    && inet (hp, np -> n_net)) {
			switch (sd = rcaux (sp, hp, response)) {
			    case NOTOK: 
				continue;
			    case OOPS1: 
				break;
...
static int  rcaux (sp, hp, response)
register struct servent *sp;
register struct hostent *hp;
register char *response;
{
    ...
    if ((sd = getport (hp -> h_addrtype, response)) == NOTOK)
	return OOPS2;
...
static int getport (addrtype, response)
int	 addrtype;
register char *response;
{
    int     sd,

...
#ifndef	lint
static char ident[] = "@(#)$Id: popsbr.c,v 2.7 1994/04/19 20:12:14 jromine Exp shettich $";
#endif	lint
...
#ifndef NNTP
#ifndef	KPOP
    if ((fd1 = client (host, "tcp", POPSERVICE,  response)) == NOTOK)
#else	/* KPOP */
    (void) sprintf (buffer, "%s/%s", POPSERVICE, "kpop");
    if ((fd1 = client (host, "tcp", buffer, response)) == NOTOK)
#endif
#else	/* NNTP */
    if ((fd1 = client (host, "tcp", "nntp", response)) == NOTOK)
#endif
	return NOTOK;
